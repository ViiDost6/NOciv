...
public class AIUnitController : MonoBehaviour
{
    ...
    private float CalculateLocalScore(TileData tile)
    {
        ...
        if (HasFriendlyNeighbor(tile))
        {
            score -= 5.0f; // Valor de penalización ajustable (-2.0f a -5.0f suele funcionar bien)
        }
    ...
    private bool HasFriendlyNeighbor(TileData tile)
    {
        ...
        {
            // Comprobamos si el vecino tiene unidad Y si esa unidad NO es del jugador (es aliada de la IA)
            if (n.hasUnit && n.GetComponentInChildren<Unit>() != null && !n.GetComponentInChildren<Unit>().isPlayerUnit)
                return true;
        }
        return false;
    }
    ...
    public List<TileData> CalculatePath(TileData start, TileData end)
    {
        ...
        while (frontier.Count > 0)
        {
            ...
            foreach (var neighbor in current.neighbors)
            {
                ...
                // Evitación de colisiones dinámica:
                // Si el vecino tiene una unidad y NO es el destino final (ataque/captura), es un obstáculo.
    ...
    private void HandleCapture(TileData tile)
    {
        if (tile.currentBuilding != null)
        {
            ...
            if (tile.currentBuilding.hasBeenClaimed != team)
            {
                ...
                // CRÍTICO: Recalcular economía global
                if (TurnManager.Instance != null) TurnManager.Instance.RecalculateEconomy();

                // ELIMINADO: Incremento/Decremento manual.
                // Ahora usamos las variables actualizadas en UnitManager
                if (UnitManager.Instance.aiBaseCount <= 0 && TurnManager.Instance != null) TurnManager.Instance.EndGame(true);
                else if (UnitManager.Instance.playerBaseCount <= 0 && TurnManager.Instance != null) TurnManager.Instance.EndGame(false);
            }
        }
    }
    ...
    public Unit GetBestTargetInRange()
    {
        ...
        unit.attackableTiles = UnitManager.Instance.CalculateTilesInRange(unit.currentTile, range);
        foreach (var enemy in TurnManager.Instance.GetAllUnits(true))
        {
            if (enemy == null) continue;
            if (unit.attackableTiles.Contains(enemy.currentTile) && enemy.health < minHealth) { minHealth = enemy.health; best = enemy; }
        }
        return best;
    }
}